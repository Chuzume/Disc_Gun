...
# 水で減速
    execute if predicate discgun:in_liqud run scoreboard players set @s Chuz.Speed 4
    execute if predicate discgun:in_liqud run particle minecraft:bubble ~ ~0.3 ~ 0.25 0.25 0.25 0.1 1

# 空気で戻る
    execute unless predicate discgun:in_liqud run scoreboard players set @s Chuz.Speed 8
...
# プレイヤーの衝突判定
    execute if entity @s[tag=!D.Gun_NoHit] positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[dx=0,type=!#discgun:unhurtable,type=!ender_dragon,tag=!D.Gun_Common,sort=nearest,limit=1] at @s run function discgun:entity/disc/hit
