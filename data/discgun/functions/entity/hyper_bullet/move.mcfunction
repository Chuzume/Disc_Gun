...
# パーティクル
    particle minecraft:dust 1 0.75 0.5 0.25 ~ ~ ~ 0 0 0 0 1 force
    particle minecraft:dust 1 0.75 0.25 0.25 ~ ~ ~ 0 0 0 0 1 force
...
# プレイヤーの衝突判定
    execute if entity @s[tag=D.Gun_NoHit,tag=D.Gun_NoHitUser] positioned ~-0.5 ~0.1 ~-0.5 positioned ~0.3 ~0.0 ~0.3 if entity @e[dx=0,type=!#discgun:unhurtable,type=!ender_dragon,sort=nearest,limit=1] at @s positioned ~-0.5 ~-0.4 ~-0.5 positioned ~-0.3 ~-0.3 ~-0.3 if entity @e[dx=0,type=!#discgun:unhurtable,sort=nearest,limit=1] at @s run function discgun:entity/hyper_bullet/hit

# 対ドラゴン
    execute at @s at @e[type=minecraft:ender_dragon,team=!null,distance=..6,sort=nearest,limit=1] run function discgun:entity/hyper_bullet/hit

# 壁の衝突判定
    execute unless block ~ ~ ~ #discgun:no_collision run function discgun:entity/hyper_bullet/break
    
# 再帰
    execute if entity @s[scores={D.Gun_Recursion=1..,D.Gun_Range=1..}] at @s run function discgun:entity/hyper_bullet/move
